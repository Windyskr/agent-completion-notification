...
Scoop is maintained by `Windyskr/scoop-bucket`. After the GitHub Release is
published, manually dispatch the `Excavator` workflow; do not wait for its
scheduled run. Wait for the workflow to finish, then verify `bucket/acn.json`
